...
void quiry(CharString *maintext,int &original_position,character *root,CharStringLink &result)
{
	...
	if(currentnode->endtag==0)                      //结束查询字典
	{
		if(endposition!=0)
		{
			CharString* ncurrentword;
			ncurrentword=currentword.substring(currentword,0,2*endposition);
			original_position+=2*endposition;
			result.add(*ncurrentword);               //向分词结果的字符串链表中加入这个词
			delete ncurrentword;
		}
		else
			original_position+=2;;
...
CharStringLink* divideword2(CharString *maintext,character *root)
{
	CharStringLink *result;
	result=new CharStringLink;                             //存储分词结果的字符串链表
	int position=0;
	while(maintext->GetAChar(position)!='\0')
	{
		if(maintext->GetAChar(position)>=0&&maintext->GetAChar(position)<=127)  //自动忽略英文、数字、ASCII字符
